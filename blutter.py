...
class BlutterInput:
    def __init__(self, libapp_path: str, dart_info: DartLibInfo, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, show_analysis_log: bool):
        self.libapp_path = libapp_path
        self.dart_info = dart_info
        self.outdir = outdir
        self.rebuild_blutter = rebuild_blutter
        self.create_vs_sln = create_vs_sln
        self.show_analysis_log = show_analysis_log

        vers = dart_info.version.split('.', 2)

        # Analysis is not complete on <2.15 but it doesn't crash, some assembly errors for example:
        #    Analysis error at line ... `...`: insn.ops[1].shift.type == ARM64_SFT_LSR && insn.ops[1].shift.value == 32
        #
        # Commented out as it no longer crashes under 2.15
...
def find_compat_macro(dart_version: str, no_analysis: bool, show_analysis_log: bool):
    macros = []
    ...
    if show_analysis_log:
        macros.append('-DSHOW_ANALYSIS_LOG=1')
...
def cmake_blutter(input: BlutterInput):
    ...
    macros = find_compat_macro(input.dart_info.version, input.no_analysis, input.show_analysis_log)
    my_env = None
    ...
    # Use version-specific source directory for Dart 2.7.2 compatibility
    if '2.7.2' in input.dart_info.version:
        srcdir = 'src_v2_7_2'
    else:
        srcdir = 'src'

    # cmake -GNinja -Bbuild -DCMAKE_BUILD_TYPE=Release
    subprocess.run([CMAKE_CMD, '-GNinja', '-B', builddir, f'-DDARTLIB={input.dart_info.lib_name}', f'-DNAME_SUFFIX={input.name_suffix}', f'-DSRCDIR={srcdir}', '-DCMAKE_BUILD_TYPE=Release', '--log-level=NOTICE'] + macros, cwd=blutter_dir, check=True, env=my_env)
...
def build_and_run(input: BlutterInput):
    ...
    if input.create_vs_sln:
        ...
        # print the executable, used for debugging or running in gdb
        blutter_build_process = [input.blutter_file, '-i', input.libapp_path, '-o', input.outdir]
        print(f'Executing build process: {blutter_build_process}')
        subprocess.run(blutter_build_process, check=True)

def main_no_flutter(libapp_path: str, dart_version: str, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, show_analysis_log: bool):
    version, os_name, arch = dart_version.split('_')
    dart_info = DartLibInfo(version, os_name, arch)
    input = BlutterInput(libapp_path, dart_info, outdir, rebuild_blutter, create_vs_sln, no_analysis, show_analysis_log)
    build_and_run(input)

def main2(libapp_path: str, libflutter_path: str, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, show_analysis_log: bool):
    dart_info = get_dart_lib_info(libapp_path, libflutter_path)
    input = BlutterInput(libapp_path, dart_info, outdir, rebuild_blutter, create_vs_sln, no_analysis, show_analysis_log)
    build_and_run(input)

def main(indir: str, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, show_analysis_log: bool):
    if indir.endswith(".apk"):
        with tempfile.TemporaryDirectory() as tmp_dir:
            libapp_file, libflutter_file = extract_libs_from_apk(indir, tmp_dir)
            main2(libapp_file, libflutter_file, outdir, rebuild_blutter, create_vs_sln, no_analysis, show_analysis_log)
    else:
...
if __name__ == "__main__":
    ...
    parser.set_defaults(show_analysis_log=True)
    ...
    parser.add_argument('--hide-analysis-log', dest='show_analysis_log', action='store_false', help='Hides detailed analysis log during processing')
    # rare usage scenario
    ...
    if args.dart_version is None:
        main(args.indir, args.outdir, args.rebuild, args.vs_sln, args.no_analysis, args.show_analysis_log)
    else:
        main_no_flutter(args.indir, args.dart_version, args.outdir, args.rebuild, args.vs_sln, args.no_analysis, args.show_analysis_log)
